...
export async function fetchMarketJobs(input: { query?: string; location?: string; mode?: string; limit?: number } = {}): Promise<JobFeed> {
  ...
  try {
    ...
    const response = await fetch(`${url}/rest/v1/rpc/search_job_market`, {
      method: "POST",
...
async function fetchFallback(input: { query?: string; location?: string; mode?: string; limit?: number }): Promise<JobFeed> {
...
function fromDatabase(job: DatabaseJob): Job {
  return { ...job, postedAt: job.posted_at, sourceUrl: job.source_url, employmentType: job.employment_type };
}
